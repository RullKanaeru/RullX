...
let handler = async (m, { conn }) => {
  ...
  const caption = `*📌 ɴᴀᴍᴀ:* ${user.registered ? user.name : conn.getName(m.sender)}
*▸ ᴇxᴘ:* ${user.exp}
*▸ ʟᴇᴠᴇʟ:* ${user.level}
*▸ ʀᴏʟᴇ:* ${user.role}
*▸ ɴʏᴀᴡᴀ:* ${user.health}
*▸ ʟɪᴍɪᴛ:* ${user.limit}
*▸ ᴜᴀɴɢ:* ${user.money}${user.atm ? `
*▸ ᴀᴛᴍ:* ʟᴠ.${user.atm}
*▸ ʙᴀɴᴋ:* ${user.bank} $ / ${user.fullatm} $`: ''}

▸ *sᴛᴀᴛᴜs:* ${user.premiumTime > 0 ? 'Premium' : 'Free'}
▸ *ᴛᴇʀᴅᴀғᴛᴀʀ:* ${user.registered ? 'Yes':'No'}${user.premiumTime >= 1 ? `
▸ *ᴇxᴘɪʀᴇᴅ:*
${clockString(user.premiumTime - new Date() * 1)}`: ''}
`.trim()
  conn.sendButton(m.chat, `${htki} ᴜ s ᴇ ʀ s ${htka}`, caption, null, [`ʀᴘɢ ɪɴᴠᴇɴᴛᴏʀʏ`, '.inv'],m)
}
handler.help = ['my']
handler.tags = ['xp']
